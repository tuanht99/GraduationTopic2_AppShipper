    ...
    useEffect(() => {
        if (lastestOrder != null) {
            const unsub = onSnapshot(
                doc(db, 'order_status', lastestOrder.status + ''),
                doc => {
                    console.log('cc', doc.data().value);
                    setOrderState(doc.data().value);
                }
            );
            ...
            >
                <View>
                    ...
                    <Text style={{ fontSize: 20 }} type="money">
                        Tiền cần thu:{' '}
                        {lastestOrder != null
                            ? lastestOrder.totalPrice -
                              lastestOrder.deposit +
                              ' VND'
                            : 'Loading... '}
                    </Text>
                </View>
                ...
                <View style={styles.locationPane}>
                    ...
                    >
                        {lastestOrder && lastestOrder.status === 2 && (
                            <TouchableOpacity
                        ...
                        {lastestOrder && lastestOrder.status === 3 && (
                            <TouchableOpacity
                                style={styles.buttonAccept}
                                onPress={() => {
                                    getOrderSuccess();
                        ...
                        {lastestOrder && lastestOrder.status === 6 && (
                            <TouchableOpacity
                                style={styles.buttonAccept}
                                onPress={() => {
                                    finishOrder();
